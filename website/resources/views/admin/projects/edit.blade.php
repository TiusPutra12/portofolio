...
<div class="animate-fade-in max-w-2xl mx-auto">
    <div class="mb-6">
        <a href="/admin/projects" class="inline-flex items-center gap-2 text-sm text-slate-500 hover:text-primary-400 transition-colors font-mono">
    ...
    <div class="admin-card p-6 sm:p-8">
        <div class="mb-6">
            <p class="font-mono text-xs text-neon-400 mb-1">// edit_project({{ $project->id }})</p>
            <h2 class="text-xl font-bold text-white">Edit Proyek</h2>
        </div>
        ...
        <form method="POST" action="/admin/projects/{{ $project->id }}" enctype="multipart/form-data" class="space-y-6">
            ...
            <div>
                ...
                <input type="text" name="title" id="title" value="{{ old('title', $project->title) }}" required
                    class="input-field" placeholder="Contoh: Sistem Informasi Perpustakaan">
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="3"
                    class="input-field" placeholder="Deskripsi singkat proyek...">{{ old('description', $project->description) }}</textarea>
            ...
            <div>
                ...
                <input type="date" name="date" id="date" value="{{ old('date', $project->date->format('Y-m-d')) }}" required
            ...
            <div>
                ...
                <input type="url" name="url" id="url" value="{{ old('url', $project->url) }}"
                    class="input-field" placeholder="https://github.com/username/project">
                @error('url') <p class="text-red-400 text-xs mt-1.5 font-mono">{{ $message }}</p> @enderror
            </div>
            ...
            <div>
                ...
                @if($project->image)
                    <div class="mb-3 relative rounded-lg overflow-hidden border border-slate-800 w-full sm:w-1/2 aspect-video bg-[#161b22]">
                        <img src="{{ asset('storage/' . $project->image) }}" alt="Current image" class="w-full h-full object-cover">
                    </div>
                @endif
                <div class="relative">
            ...
            <div class="pt-6 border-t border-slate-800 flex items-center justify-end gap-3">
                <a href="/admin/projects" class="px-5 py-2 rounded-lg text-slate-400 hover:text-white transition-colors text-sm font-mono border border-slate-800 hover:border-slate-600 bg-[#161b22]">
                    cancel()
                </a>
