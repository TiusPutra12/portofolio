...
<head>
    ...
    <meta name="description" content="Portfolio Developer — Sertifikat, Kegiatan & Proyek Terbaik Saya">
    <title>{{ config('app.name', 'Dev Portfolio') }}</title>
            ...
            <div class="flex items-center justify-between h-16 sm:h-20">
                ...
                <div class="hidden md:flex items-center gap-1">
                    @foreach([['#about','about()'],['#projects','projects[]'],['#certificates','certs{}'],['#activities','timeline()']] as [$href,$label])
                        <a href="{{ $href }}" class="px-4 py-2 text-xs font-mono font-medium text-slate-400 hover:text-primary-400 hover:bg-primary-950/50 rounded-lg transition-all duration-200 border border-transparent hover:border-primary-800/50">
                            {{ $label }}
                        </a>
                    @endforeach
                </div>
                ...
                {{-- Mobile menu btn --}}
                <button id="mobile-menu-btn" class="md:hidden p-2 rounded-lg text-slate-400 hover:text-primary-400 hover:bg-primary-950/50 transition-all border border-transparent hover:border-primary-800/50">
